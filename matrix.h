...
float matrix_getelem(MatrixList* m, int x, int y)
{
      ...
      // Escaneia a matriz dentro dos nós já existentes buscando se suas coordenadas batem
...
void matrix_setelem(MatrixList* m, int x, int y, float elem)
{
      ...
      // Checa se, nas coordenadas solicitadas, já existe algum nó
      while (currentNode != m -> head)
      {
            if (currentNode -> line == x - 1 && currentNode -> column == y - 1)
            {
...
MatrixList* matrix_create() 
{
      ...
      // Delimita o tamanho da matriz e deixa salvo na MatrixList
      ...
      // Cria o nó cabeça e inicia os ponteiros
      ...
      int line, column;
      float info;
      char input[1000]; // a linha, coluna e a info nao podem ocupar um espaço maior que 1000 caracteres;
...
void matrix_destroy(MatrixList* m) 
{
      ...
      free(m -> head); //Libera a "Matrix"

      free(m); //Libera a "MatrixList"
}
